function Configuration(options, args) {
  ...
  var self = {
    ...
    getRuntimeFilter: function getRuntimeFilter() {
      var tagGroups = Cucumber.TagGroupParser.getTagGroupsFromStrings(options.tags);
      var tagRules = tagGroups.map(function (tags) {
         return Cucumber.Runtime.Filter.AnyOfTagsRule(tags);
      });
      var lineRule = Cucumber.Runtime.Filter.ScenarioAtLineRule(args);
      var rules = tagRules.concat([lineRule]);
      return Cucumber.Runtime.Filter(rules);
    },
