function PrettyFormatter(options) {
  ...
  self.handleBeforeScenarioEvent = function handleBeforeScenarioEvent(event, callback) {
    ...
    if (options.showSource) {
      ...
      identifier = self._pad(identifier, uriCommentIndex + 2) +
                   colors.comment('# ' + path.relative(process.cwd(), uri) + ':' + scenario.getLine());
    }
    source += identifier;
  ...
  self.logStepResult = function logStepResult(step, stepResult) {
    ...
    if (options.showSource && step.hasUri()) {
      ...
      identifier += colors.comment('# ' + path.relative(process.cwd(), uri) + ':' + step.getLine());
    }
    ...
    step.getArguments().forEach(function (argument) {
      var str;
      if (typeof argument === 'string') {
        str = self.formatDocString(stepResult, argument);
      } else if (typeof argument === 'object' && typeof argument.raw === 'function') {
        str = self.formatDataTable(stepResult, argument);
      } else {
        throw new Error('Unknown argument type: ' + argument);
      }
