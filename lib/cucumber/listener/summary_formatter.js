function SummaryFormatter(options) {
  ...
  self.storeFailedScenario = function storeFailedScenario(failedScenario) {
    var name        = failedScenario.keyword + ': ' + failedScenario.name;
    var relativeUri = path.relative(process.cwd(), failedScenario.path);
    var line        = failedScenario.locations[0].line;
    self.appendStringToFailedScenarioLogBuffer(relativeUri + ':' + line + ' # ' + name);
  };
