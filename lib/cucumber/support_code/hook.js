function Hook(code, options) {
  ...
  self.appliesToScenario = function appliesToScenario(scenario) {
    var runtimeFilter = self.getRuntimeFilter();
    return runtimeFilter.isElementEnrolled(scenario);
  };

  self.getRuntimeFilter = function getRuntimeFilter() {
    var tagGroups = Cucumber.TagGroupParser.getTagGroupsFromStrings(tags);
    var rules = _.map(tagGroups, function (tagGroup) {
      var rule = Cucumber.Runtime.Filter.AnyOfTagsRule(tagGroup);
      return rule;
    });
    var runtimeFilter = Cucumber.Runtime.Filter(rules);
