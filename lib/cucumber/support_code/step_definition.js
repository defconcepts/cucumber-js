function StepDefinition(pattern, options, code) {
  ...
  var self = {
    ...
    buildInvocationParameters: function buildInvocationParameters(step, scenario, callback) {
      ...
      parameters.shift();
      parameters = parameters.concat(step.arguments.map(function (stepArgument) {
        if (stepArgument.content) {
          return stepArgument.content;
        } else if (stepArgument.rows) {
          return Cucumber.Runtime.DataTable(stepArgument.rows);
        }
      }));
